...
- **wp-env**: Install Composer dependencies and run `npm run build` before starting the development environment with `npm run env:start`; the lifecycle only activates Demo Plugin. Missing Composer autoloaders produce a development warning and prevent the plugin from loading. Application tests use the isolated `.wp-env.test.json` environment. Use the `wp-plugin-bp` skill when tests are involved.
