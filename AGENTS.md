...
- **wp-env**: Start the development environment with `npm run env:start`. Missing generated Composer autoloaders are installed inside wp-env before plugin activation, so host PHP and Composer are not required. Compiled assets are committed in `build/`, so startup does not rebuild them. Application tests use the isolated `.wp-env.test.json` environment. Use the `wp-plugin-bp` skill when tests are involved.
