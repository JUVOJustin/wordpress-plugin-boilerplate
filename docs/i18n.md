...
3. (Optional) Run `composer run i18n:compile` to compile the `.po` files into `.mo`, `.json` and `.php` files for use by WordPress. The `deploy` action does this automatically using [wp-env](wp-env.md).
