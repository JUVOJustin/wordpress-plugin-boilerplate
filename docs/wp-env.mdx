...
:::note
WP-CLI is treated as an environment dependency. Commands such as `composer run i18n:extract` and `composer run i18n:compile` assume that `wp` and the required WP-CLI packages are already available in the active environment. `wp-env` installs the WP-CLI i18n package automatically during `env:start`. If you run the commands outside `wp-env`, install WP-CLI yourself and ensure the needed packages are available. See the [official WP-CLI installation guide](https://make.wordpress.org/cli/handbook/guides/installing/).
:::
...
The boilerplate uses wp-env in the deploy workflow to compile translations. This requires a full WordPress environment with WP-CLI available. Because the i18n package is installed through `wp-env`, the deploy workflow can rely on the same translation setup as local development.
...
- The `afterStart` lifecycle script installs the pinned i18n package in the `cli` container and activates the plugin
