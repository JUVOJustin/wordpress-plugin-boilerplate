...
See `.wp-env.json` for the configuration. The setup uses `mappings` instead of the common `plugins: ["."]` approach and runs the containers with PHP 8.1 so WP-CLI i18n commands support the `make-php` compilation step.

The test configuration prefers port 8889; locally, both configurations fall back to an available port.
...
| `npm run env:test:cli` | Run commands inside the application-test environment |
...
The `afterStart` hook only activates Demo Plugin.
...
- Ports are selected automatically and may differ between starts
