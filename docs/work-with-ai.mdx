...
description: Commands, skills, and instructions for AI-assisted development.
---
...
| `/wp-skills-upsert [skillnames]` | Download and install official WordPress agent skills. Default: `wp-interactivity-api`, `wp-project-triage`, `wp-block-development`, `wp-phpstan`, `wp-rest-api`. |
...
See [i18n](i18n.mdx) for detailed translation documentation.
...
- Deterministic placeholder replacement via `scripts/boilerplate-replace.php`

See [boilerplate-update skill](../.agents/skills/boilerplate-update) for the full workflow.

### application-testing

Guides the AI when writing or extending PHPUnit application tests:
- Enforces the wp-env-exclusive test setup
- Applies correct conventions (file location, class naming, no namespace)
- Uses WordPress factories for test data and explains automatic rollback
- Handles plugin dependencies via `lifecycleScripts`

See [Testing](testing.mdx) for the full testing documentation.

### Skills are not available?
Ask your agent to clone the skills or use skills.sh to install them.

```bash title="Terminal"
npx skills add https://github.com/JUVOJustin/wordpress-plugin-boilerplate
```
...
- Commands are basically prompts. You can copy their body to any tool. You might need to adjust the `frontmatter` header.
